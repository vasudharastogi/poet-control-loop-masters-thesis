...
# Compute MEDIANMAPE time series across multiple predicted qs2 directories
...
# Compute MEDIANMAPE series for one predicted input vs reference
compute_series_medianmape <- function(ref_in, pred_in, species_list) {
  ...
  series <- data.frame(iter = integer(), medianmape = numeric(), stringsAsFactors = FALSE)
  ...
  for (i in seq_len(nrow(pairs))) {
    ...
    series <- rbind(series, data.frame(iter = iter, medianmape = safe_median(mape_vec)))
  }
...
if (length(args) < 3) {
  stop(paste(
    "Usage:",
    "  Rscript multi_medianmape_timeseries.R <phreeqc_ref_dir_or_glob> <reference_pred_dir_or_glob> <pred1_dir_or_glob> [pred2 ...]",
    "",
    "Meaning:",
    "  1) phreeqc_ref      = PHREEQC reference (denominator for MAPE for ALL lines)",
    "  2) reference_pred   = special prediction to plot in RED (compared vs phreeqc_ref)",
    "  3+) preds           = other predictions (compared vs phreeqc_ref)",
    sep = "\n"
  ))
}
...
pred_list   <- args[-c(1, 2)]   # all other predictions
...
s_ref <- compute_series_medianmape(ref_in, ref_pred_in, species_list)
...
# All other curves: pred_in vs PHREEQC ref_in
for (pred_in in pred_list) {
  cat("\nComputing:", pred_in, "vs", ref_in, "\n")
  s <- compute_series_medianmape(ref_in, pred_in, species_list)
...
# Plot (base R)
# ----------------------------
output_pdf <- "multi_medianmape.pdf"
pdf(output_pdf, width = 5, height = 4)

par(family = "serif", mar = c(4, 4, 2, 2))
...
# decade ticks: 1e-3, 1e-2, 1e-1, 1e0, ...
y_ticks <- 10^seq(
  floor(log10(ylim[1])),
...
cols <- grDevices::hcl.colors(length(keys), palette = "Greens", rev = TRUE) 
names(cols) <- keys 
