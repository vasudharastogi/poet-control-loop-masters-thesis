...
int parseInitValues(int argc, char **argv, RuntimeParameters &params) {
  ...
  try {
    ...
    *global_rt_setup = global_rt_setup->operator[]("value");
    ...
    params.ctrl_cell_ids =
        Rcpp::as<std::vector<uint32_t>>(global_rt_setup->operator[]("ctrl_cell_ids"));
...
static void getControlCellIds(std::vector<std::uint32_t> &ids, int root, MPI_Comm comm) {
